...
侧栏 **接入与使用** 提供按当前配置生成的 MCP 地址、OAuth 填写步骤和首条只读试用指令；也可直接打开 `/console/#guide`。它只显示配置并提供教程，不会创建隧道、操作 ChatGPT 账号或将服务存活误报为已连接。项目支持名称／路径搜索，展开的任务分页会在刷新后保留；断线时保留同一视图的最近结果并显示重试入口。
