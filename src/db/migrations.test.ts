...
test("upstream workspace recovery appends to the fork's existing migration history", () => {
  ...
  try {
    ...
    db.exec("drop table console_thread_references; alter table workspace_sessions drop column recovery_kind; delete from devspace_schema_migrations where version>=13;");
