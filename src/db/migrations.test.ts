...
test("upstream workspace recovery appends to the fork's existing migration history", () => {
  ...
  try {
    ...
    db.exec("alter table workspace_sessions drop column recovery_kind; delete from devspace_schema_migrations where version=13;");
    ...
    migrateDatabase(db);
    assert.equal((db.prepare("select count(*) as n from devspace_schema_migrations").get() as { n: number }).n, 13);
  } finally { db.close(); }
});
